...
<link href="quizpet.css" rel="stylesheet" type="text/css" />
...
<%      
        ...
        String onlineCompileDir = getServletContext().getRealPath("online_compile") + dirDelim;//appDir + 
        String extraClassDir = getServletContext().getRealPath("class") + dirDelim;//appDir + 
        String curActionDir = "";
...
<pre><%  
		...
		Pattern p = Pattern.compile("_eval\\((.*?)\\)");
		Matcher m = p.matcher(codeStr);
		while(m.find()) {
		    String evalStr = m.group(1);
		    ScriptEngineManager manager = new ScriptEngineManager();
		    ScriptEngine engine = manager.getEngineByName("js");
		    Object result = engine.eval(evalStr);
		    codeStr = codeStr.replace("_eval(" + evalStr + ")", result.toString());
		}
       ...
       String submitString = "<input type=submit value=Submit name=submitButton onclick=submitFunction(this.form,1)>";// + "," + evalAnsServlet + ")>";
       //System.out.println("submitString:\n" + submitString);
       if(quesType==1){
           out.println("<font color=blue>What is the final value of <b>result</b>?</font><br>");
           out.println("<input type=text name=inputans size=5>" + submitString);
       }else if (quesType==2){
	      out.println("<font color=blue>What is the output?</font><br><font color=grey size=2>Be careful of the whitespace(space,newline) in your answer.</font><br>");
          out.println("<textarea name=inputans rows=5 cols=45></textarea>" + submitString);
       }
       else if (quesType==3){
...
%>
		    ...
		    %> <h2 class="tab"><%=fileName.get(j).toString()%></h2>
		    <%
		    }
		    %>
		    <pre><%
